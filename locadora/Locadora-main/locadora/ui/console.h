...
// Aguarda o usuario digitar 0 para voltar (mesma convencao do "0 = sair").
inline void pausar() {
    while (true) {
        std::cout << "\n" << Cor::CINZA << "  [0] Voltar: " << Cor::RESET;
        std::string linha;
        if (!std::getline(std::cin, linha)) return;   // EOF: encerra a leitura
        // remove espacos das pontas
        size_t ini = linha.find_first_not_of(" \t\r");
        size_t fim = linha.find_last_not_of(" \t\r");
        std::string limpo = (ini == std::string::npos) ? "" : linha.substr(ini, fim - ini + 1);
        if (limpo == "0") return;
    }
}
...
// Ano corrente do relogio do sistema (para validar ano de cadastro).
inline int anoAtual() {
